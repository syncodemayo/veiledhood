...
export const agentRunInputShape = { id: z.string().min(8) };
export const agentRunInputSchema = z.object(agentRunInputShape);
...
export async function handleAgentRun(input: AgentRunInput): Promise<McpToolResponse> {
  try {
    const { aesKey } = await loadMasterKey();
    const res = await apiRequest<AgentDoc>(
      `/agents/${encodeURIComponent(input.id)}/run`,
      { method: "POST" },
    );
    const doc = res.data;
    ...
    return new VeiledhoodMcpError(
      "VEILEDHOOD_UNKNOWN",
      `agent_run failed: ${e instanceof Error ? e.message : String(e)}`,
    ).toMcpContent();
  }
}
